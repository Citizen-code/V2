...
export default async function Page({params}:{params:{id:string}}) {
  ...
  const test = await prisma.test_result.findFirst({where:{id:params.id, employee_id:session?.user?.id, test:{type_id:1}}, include:{test:{include:{test_questions:true}}, result_questions:true}})
