...
export default function TestEdit({ levels, categories, type_test, selectedTest }: { levels: level[], categories: category[], type_test: type_test[], selectedTest: (test & { level: level, category: category, type_test: type_test, test_public:test_public | null })}) {
  ...
  async function onSubmit(values: testSchemaType) {
    try {
      ...
      toast({
        title: "Успешно",
        description: "Данные успешно изменены. Для отображения изменений обновите страницу.",
      });
    } catch (error) {
      toast({
        title: "Ошибка",
