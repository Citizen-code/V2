...
export default function UserEdit({ levels, positions, open, setOpen, selectedUser, load }: { load:() => void,levels: level[], positions: position[], open: boolean, setOpen: Dispatch<SetStateAction<boolean>>, selectedUser: (employee & { employee_level: employee_level[], employee_position: employee_position[] }) | undefined }) {
  ...
  async function onSubmit(values: userSchemaType) {
    try {
      await EditUser(values, selectedUser!.id)
      toast({
        title: "Успешно",
                                    ...
                                    <CommandList>
                                      {levels.filter(i => field.value.findIndex(el => el.level_id === i.id) === -1)?.map((i) => (
                                        <CommandItem value={i.name} key={i.name} onSelect={() => {
                                          form.setValue('employee_level', [...field.value, { employee_id: selectedUser.id, level_id: i.id, date: new Date() }])
                                  ...
                                  <CommandGroup>
                                    <CommandList>
                                      {positions.filter(i => field.value.findIndex(el => el.position_id === i.id) === -1)?.map((i) => (
                                        <CommandItem value={i.name} key={i.name} onSelect={() => {
                                          form.setValue('employee_position', [...field.value, { employee_id: selectedUser.id, position_id: i.id }])
